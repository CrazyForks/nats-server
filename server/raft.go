...
func (n *raft) Propose(data []byte) error {
	...
	if n.isLeaderOverrun() {
		...
		if n.stepdownIfLeaderOverrunLocked() {
			n.Unlock()
			return errNotLeader
		}
		n.Unlock()
...
// Lock should be held.
func (n *raft) stepdownIfLeaderOverrunLocked() bool {
	if !n.isLeaderOverrun() {
		return false
	}
	var state StreamState
...
func (n *raft) isLeaderOverrun() bool {
	...
	// Or the number of unapplied entries is over the threshold: upper layer is not applying our commits.
